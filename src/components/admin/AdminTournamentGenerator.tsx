...
import { Shuffle, Save, Calendar, Check, LayoutGrid } from 'lucide-react';
...
export default function AdminTournamentGenerator() {
  ...
  const [customGroups, setCustomGroups] = useState<number>(2);
  const [customMatches, setCustomMatches] = useState<number>(12);
  ...
  useEffect(() => {
    ...
    if (selectedFormat === 'romano') {
      ...
      const contextPairs: Pair[] = registeredPairs.map((rp, index) => ({
  ...
  }, [customPairs, registeredPairs, registeredPlayers, selectedFormat]);
  ...
  const generateFixture = () => {
    ...
    setTimeout(() => {
      ...
      if (selectedFormat === 'personalizado') {
        ...
        // Americano & Romano
        const pairsPerGroup = Math.ceil(customPairs / customGroups);
      
      for (let c = 0; c < customGroups; c++) {
        const courtPairs = sortedPairs.slice(c * pairsPerGroup, c * pairsPerGroup + pairsPerGroup).map((p, idx) => ({
          ...p,
          localId: idx + 1
        }));

        // 3. Generar cruces de forma secuencial
        // Como las parejas no siempre son 4, hacemos un "round robin" básico para los partidos
        const m = courtPairs;
        const matches: CourtMatch[] = [];
        const matchesForThisGroup = Math.ceil(customMatches / customGroups);
        
        // Creamos pares intentando cruzar a todos
        let matchCount = 0;
        for (let i = 0; i < m.length; i++) {
          for (let j = i + 1; j < m.length; j++) {
            if (matchCount < matchesForThisGroup) {
              matches.push({
                round: matchCount + 1,
                team1: m[i],
                team2: m[j]
        ...
        // Si faltan partidos, repetimos los primeros (simulando rotación)
        let fallbackIndex = 0;
        while (matchCount < matchesForThisGroup && matches.length > 0) {
           matches.push({
             round: matchCount + 1,
             team1: matches[fallbackIndex % matches.length].team1,
             team2: matches[fallbackIndex % matches.length].team2
           });
           matchCount++;
           fallbackIndex++;
        }
  ...
  const handleSave = () => {
    ...
    createTournament(
      `${selectedFormat.charAt(0).toUpperCase() + selectedFormat.slice(1)} de ${selectedFormat === 'mexicano' ? Math.floor(customPairs/2) + ' Canchas' : selectedFormat === 'personalizado' ? customPairs + ' Jugadores' : customGroups + ' Grupos'}`, 
      selectedFormat,
      selectedFormat === 'mexicano' ? Math.floor(customPairs/2) : selectedFormat === 'personalizado' ? Math.floor(customPairs/4) : customGroups, 
      customPairs, 
      participants, 
      fixture || undefined,
      ...
      <div className="mx-auto w-full relative z-10">
        ...
        >
          ...
          <p className="text-slate-400 font-light text-sm">
            Crea fixtures al instante. Las parejas manuales y los jugadores individuales registrados aparecerán aquí para formar el fixture.
          </p>
        </motion.div>
        ...
        {!fixture && !isGenerating && (
          ...
          >
            ...
            <div className="flex bg-slate-900/60 p-1.5 rounded-2xl w-full mb-6 border border-slate-800 backdrop-blur-sm">
              <button
                ...
                className={`flex-1 py-3 text-sm font-bold tracking-widest uppercase transition-all rounded-xl ${
                  selectedFormat === 'americano' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'
                }`}
              >
                Americano
              </button>
              <button
                ...
                className={`flex-1 py-3 text-sm font-bold tracking-widest uppercase transition-all rounded-xl ${
                  selectedFormat === 'mexicano' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'
                }`}
              >
                Mexicano
              </button>
              <button
                ...
                className={`flex-1 py-3 text-sm font-bold tracking-widest uppercase transition-all rounded-xl ${
                  selectedFormat === 'romano' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'
                }`}
              >
                Romano
              </button>
            ...
            <div className="bg-slate-900/60 border border-slate-800 p-8 rounded-3xl w-full flex flex-col items-center gap-8 mb-8 backdrop-blur-sm">
              {/* Context Hint */}
              ...
              {selectedFormat === 'mexicano' ? (
                <div className="flex flex-col items-center gap-6 w-full">
                  <div className="flex flex-col md:flex-row justify-center w-full gap-4 px-6 py-6 bg-blue-500/5 border border-blue-500/10 rounded-2xl">
                    <div className="flex flex-col items-center">
                      <input
                        ...
                        className="w-20 text-center text-4xl font-black text-white bg-transparent outline-none border-b-2 border-transparent hover:border-slate-800 focus:border-blue-500 transition-colors"
                      />
                      <span className="text-[12px] uppercase tracking-widest text-slate-500 font-bold mt-2">Canchas Disponibles</span>
                      <span className="text-[10px] text-blue-400 mt-1">({customPairs} parejas en total)</span>
                    </div>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  <div className="flex flex-col md:flex-row justify-between w-full gap-4 px-6 py-4 bg-blue-500/5 border border-blue-500/10 rounded-2xl">
                    <div className="flex flex-col items-center flex-1">
                      <input
                        ...
                        className="w-16 text-center text-3xl font-black text-white bg-transparent outline-none border-b-2 border-transparent hover:border-slate-800 focus:border-blue-500 transition-colors"
                      />
                      ...
                      <>
                        <div className="flex flex-col items-center flex-1">
                          <input
                            ...
                            min="1"
                            value={customGroups}
                            onChange={(e) => setCustomGroups(parseInt(e.target.value) || 1)}
                            className="w-16 text-center text-3xl font-black text-white bg-transparent outline-none border-b-2 border-transparent hover:border-slate-800 focus:border-blue-500 transition-colors"
                          />
                          <span className="text-[10px] uppercase tracking-widest text-slate-500 font-bold mt-1">Grupos</span>
                        </div>
                        <div className="flex flex-col items-center flex-1">
                          <input
                            type="number"
                            min="1"
                            value={customMatches}
                            onChange={(e) => setCustomMatches(parseInt(e.target.value) || 1)}
                            className="w-16 text-center text-3xl font-black text-white bg-transparent outline-none border-b-2 border-transparent hover:border-slate-800 focus:border-blue-500 transition-colors"
                          />
                          <span className="text-[10px] uppercase tracking-widest text-slate-500 font-bold mt-1">Partidos Totales</span>
                        </div>
                      </>
                    )}
                  </div>
                  {selectedFormat === 'personalizado' && (
                    <div className="flex flex-col w-full px-6 py-4 bg-slate-900 border border-slate-800 rounded-2xl gap-2 mt-2">
                      ...
                      <select 
                        ...
                        className="bg-slate-950 border border-slate-700 text-white rounded-lg p-2 text-sm outline-none focus:border-blue-500"
                      >
              ...
              {selectedFormat === 'personalizado' ? (
                ...
                registeredPairs.length < customPairs && (
                  <div className="w-full bg-red-500/10 border border-red-500/30 text-red-400 p-4 rounded-xl text-center text-sm font-medium">
                    Atención: Necesitas {customPairs} parejas para este formato, pero solo tienes {registeredPairs.length} registradas. ¡Usa el panel de abajo para inscribir a las parejas faltantes!
                  </div>
                )
              )}

              <div className="w-full mt-4 mb-4 border-t border-slate-800/50 pt-4">
                <ManualRegistrationView isEmbedded />
              </div>

              <motion.button
                onClick={generateFixture}
                disabled={selectedFormat === 'personalizado' ? registeredPlayers.length < customPairs : selectedFormat === 'romano' ? registeredPlayers.length < customPairs * 2 : registeredPairs.length < customPairs}
                whileHover={{ scale: (selectedFormat === 'personalizado' ? registeredPlayers.length < customPairs : selectedFormat === 'romano' ? registeredPlayers.length < customPairs * 2 : registeredPairs.length < customPairs) ? 1 : 1.02 }}
                whileTap={{ scale: (selectedFormat === 'personalizado' ? registeredPlayers.length < customPairs : selectedFormat === 'romano' ? registeredPlayers.length < customPairs * 2 : registeredPairs.length < customPairs) ? 1 : 0.98 }}
                className={`w-full relative px-8 py-4 font-bold tracking-[0.15em] uppercase text-sm overflow-hidden flex items-center justify-center gap-3 rounded-xl transition-all ${
                  (selectedFormat === 'personalizado' ? registeredPlayers.length < customPairs : selectedFormat === 'romano' ? registeredPlayers.length < customPairs * 2 : registeredPairs.length < customPairs)
        ...
        {isGenerating && (
          <div className="flex flex-col items-center justify-center py-32 gap-6">
            ...
            <p className="text-slate-400 font-light text-lg animate-pulse">
              Distribuyendo {customPairs} parejas en {customGroups} grupos...
            </p>
          </div>
        )}
            ...
            <div className="flex overflow-x-auto gap-6 pb-8 snap-x scrollbar-hide">
              {fixture.map((group) => (
                <div 
                  ...
                  className="flex-shrink-0 w-[90vw] md:w-[400px] snap-start bg-[#0B1120] border border-slate-800 rounded-3xl overflow-hidden flex flex-col"
                >
                  {/* Court Header */}
                  <div className="bg-slate-900/80 px-6 py-5 border-b border-slate-800 flex items-center justify-between">
                    <h3 className="text-xl font-bold text-white tracking-tight flex items-center gap-2">
                      <Calendar className="w-5 h-5 text-blue-500" />
                      {group.courtName}
                    </h3>
                    <span className="text-[10px] uppercase font-bold tracking-widest text-slate-500 bg-slate-950 px-2.5 py-1 rounded-md border border-slate-800">
                      Nivel Similar
                    </span>
                  </div>
                  ...
                  <div className="p-6 flex flex-col gap-8 flex-1">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      {group.pairs.map((pair) => (
                        <div key={pair.id} className="grid grid-cols-12 items-center bg-slate-900/50 rounded-xl p-2.5 border border-slate-800/50 hover:bg-slate-800/50 transition-colors">
                          <div className="col-span-2 flex justify-center">
                            <span className="w-6 h-6 rounded-md bg-blue-500/10 text-blue-400 font-black text-xs flex items-center justify-center">
                              {pair.localId}
                            </span>
                          </div>
                    ...
                    <div className="flex flex-col gap-3 relative">
                      ...
                      {group.matches.map((match) => (
                        <div key={match.round} className="flex items-stretch bg-slate-900/40 rounded-xl border border-slate-800 overflow-hidden hover:border-slate-700 transition-colors group/match">
                          <div className="w-10 bg-slate-900 flex items-center justify-center border-r border-slate-800 shrink-0">
                            <span className="text-[10px] font-black text-slate-500 group-hover/match:text-blue-400 transition-colors">
                              R{match.round}
                            </span>
                          </div>
                          <div className="flex-1 flex items-center justify-between p-3">
                            ...
                            <div className="flex items-center gap-3 w-[40%]">
                              <span className="w-6 h-6 shrink-0 rounded bg-slate-800 flex items-center justify-center text-xs font-bold text-white shadow-sm">
                                {match.team1.localId}
                              </span>
                              <div className="flex flex-col w-full">
                                <span className="text-[11px] font-medium text-slate-300 line-clamp-1">{match.team1.player1.name.split(' ')[0]}</span>
                                <span className="text-[11px] font-medium text-slate-300 line-clamp-1">{match.team1.player2.name.split(' ')[0]}</span>
                              </div>
                            </div>

                            {/* VS */}
                            <div className="shrink-0 flex items-center justify-center w-[20%]">
                              <span className="text-[9px] font-black tracking-widest text-slate-600 bg-slate-950 px-2 py-0.5 rounded border border-slate-800">VS</span>
                            </div>

                            {/* Team 2 */}
                            <div className="flex items-center justify-end gap-3 w-[40%] text-right">
                              <div className="flex flex-col w-full">
                                <span className="text-[11px] font-medium text-slate-300 line-clamp-1">{match.team2.player1.name.split(' ')[0]}</span>
                                <span className="text-[11px] font-medium text-slate-300 line-clamp-1">{match.team2.player2.name.split(' ')[0]}</span>
                              </div>
                              <span className="w-6 h-6 shrink-0 rounded bg-slate-800 flex items-center justify-center text-xs font-bold text-white shadow-sm">
                                {match.team2.localId}
                              </span>
